...
/* TP1 - Exercice 2 : Requêtes SQL */

/* 1. Afficher la structure de la relation section et son contenu */
DESC section;
SELECT * 
FROM section;

/* 2. Afficher tous les renseignements sur les cours */
SELECT * 
FROM course;
...
/* 6. Afficher les noms des enseignants ayant un salaire > 65000 */
SELECT name
FROM teacher
WHERE salary > 65000;

/* 7. Afficher les noms des enseignants ayant un salaire entre 55000 et 85000 */
SELECT name
FROM teacher
WHERE salary BETWEEN 55000 AND 85000;

/* 8. Afficher les noms des départements, sans doublons, à partir de teacher */
SELECT DISTINCT dept_name
FROM teacher;

/* 9. Enseignants du département informatique ayant un salaire > 65000 */
SELECT name
FROM teacher
WHERE dept_name = 'Comp. Sci.'
  AND salary > 65000;

/* 10. Tous les renseignements sur les cours proposés au Spring 2010 */
SELECT *
FROM section
WHERE semester = 'Spring'
  AND year = 2010;

/* 11. Titres des cours du département informatique avec plus de 3 crédits */
SELECT title
FROM course
WHERE dept_name = 'Comp. Sci.'
  AND credits > 3;

/* 12. Noms des enseignants, leur département, et le bâtiment du département */
...
FROM student s, takes tk, course c
WHERE s.ID = tk.ID
  AND tk.course_id = c.course_id
  AND c.dept_name = 'Comp. Sci.';

/* 14. Étudiants ayant suivi un cours dispensé par Einstein */
SELECT DISTINCT s.name
FROM student s, takes tk, teaches te, teacher t
WHERE s.ID = tk.ID
  AND tk.course_id = te.course_id
...
/* 15. Identifiants des cours et enseignants qui les ont assurés */
SELECT te.course_id, t.name
FROM teaches te, teacher t
WHERE te.ID = t.ID;

/* 16. Nombre d'inscrits pour chaque enseignement proposé au Spring 2010 */
...
/* 17. Noms des départements et salaire maximum de leurs enseignants */
SELECT dept_name, MAX(salary) AS salaire_max
FROM teacher
GROUP BY dept_name;

/* 18. Nombre d'inscrits pour chaque enseignement proposé */
SELECT course_id, sec_id, semester, year, COUNT(*) AS nb_inscrits
FROM takes
GROUP BY course_id, sec_id, semester, year;

/* 19. Nombre total de cours dans chaque bâtiment
       pendant Fall 2009 et Spring 2010 */
...
/* 20. Nombre total de cours dispensés par chaque département
       et ayant eu lieu dans le bâtiment de ce département */
SELECT d.dept_name, COUNT(*) AS nb_cours
FROM department d, teacher t, teaches te, section s
WHERE d.dept_name = t.dept_name
  ...
  AND te.year = s.year
  AND d.building = s.building
GROUP BY d.dept_name;

/* 21. Titres des cours proposés et enseignants qui les ont assurés */
SELECT c.title, t.name
FROM course c, section s, teaches te, teacher t
...
/* 22. Nombre total de cours pour chaque période */
SELECT semester, COUNT(*) AS nb_cours
FROM section
GROUP BY semester;

/* 23. Pour chaque étudiant, total des crédits obtenus
       en suivant des cours hors de son département */
...
/* 24. Pour chaque bâtiment, total des crédits des cours qui y ont eu lieu */
